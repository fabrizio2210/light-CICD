...
export const projectsettings = {
  ...
  mutations: {
    ...
    updatingSuccess(state, p) {
      ...
      Vue.set(
        state.all.settings_dicts[project_id]["settings"],
        setting.name,
        setting
      );
    },
    ...
    getAllSuccess(state, p) {
      ...
      settings.forEach(function(element) {
        ...
        !(project_id in state.all.settings_dicts) &&
          Vue.set(state.all.settings_dicts, project_id, {});
        !("settings" in state.all.settings_dicts[project_id]) &&
          Vue.set(state.all.settings_dicts[project_id], "settings", {});
        Vue.set(
          state.all.settings_dicts[project_id]["settings"],
          element.name,
          element
